...
import { collection, doc, onSnapshot, addDoc, updateDoc, deleteDoc, setDoc, getDoc, runTransaction, getDocs, writeBatch, query, where } from "firebase/firestore";
...
function isGroupAdmin(group, userId) { return group?.adminId === userId; }
function isPoolAdmin(pool, userId) { return pool?.adminId === userId; }
function getShareUrl(shareId) { return `${window.location.origin}${window.location.pathname}?pool=${shareId}`; }
function isGroupMember(group, user) {
  if (!group || !user) return false;
  if (isGroupAdmin(group, user.uid)) return true;
  const memberList = group.memberList || [];
  return memberList.some(m => m.email?.toLowerCase() === user.email?.toLowerCase());
}
...
function Icon({ name, size = 18, color }) {
  ...
  const icons = {
    ...
    userCheck: <svg {...p}><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="8.5" cy="7" r="4" /><polyline points="17 11 19 13 23 9" /></svg>,
...
function AdminTransferModal({ t, entity, entityType, user, addToast, addNotif, onClose }) {
  const [generatedKey, setGeneratedKey] = useState(null);
  const [inputKey, setInputKey] = useState("");
  const [loading, setLoading] = useState(false);
  const [copied, setCopied] = useState(false);
  const isAdmin = entity?.adminId === user?.uid;

  async function handleGenerateKey() {
    if (!isAdmin) { addToast("Only the current admin can generate keys", "error"); return; }
    setLoading(true);
    try {
      const newKey = genAccessKey();
      ...
      addToast("Access key generated (valid for 24 hours)");
  ...
  async function handleUseKey() {
    if (!inputKey.trim()) { addToast("Please enter an access key", "error"); return; }
    setLoading(true);
    try {
      const q2 = query(collection(db, "accessKeys"),
      ...
      if (snap.empty) { addToast("Invalid or expired access key", "error"); setLoading(false); return; }

      const keyDoc = snap.docs[0];
      const keyData = keyDoc.data();

      if (new Date(keyData.expiresAt) < new Date()) {
        addToast("This access key has expired", "error");
        setLoading(false);
        return;
      }
      if (keyData.createdBy === user.uid) {
        addToast("You are already the admin", "error");
        setLoading(false);
        return;
      }

      const entityRef = doc(db, entityType === "group" ? "groups" : "pools", entity.id);
      const batch2 = writeBatch(db);

      batch2.update(entityRef, {
        adminId: user.uid,
        adminEmail: user.email,
      });
      batch2.update(keyDoc.ref, { used: true });

      const auditRef = doc(collection(db, "adminTransfers"));
      batch2.set(auditRef, {
        entityType,
        entityId: entity.id,
        entityTitle: entity.title,
        fromUserId: keyData.createdBy,
        fromEmail: entity.adminEmail || "unknown",
        toUserId: user.uid,
        toEmail: user.email,
      ...
      addToast(`Admin rights transferred successfully!`);
      addNotif(`You are now the admin of ${entityType} "${entity.title}"`);
      onClose();
    } catch (e) { addToast(e.message, "error"); }
    setLoading(false);
  }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 500, padding: 20 }} onClick={onClose}>
      <Card t={t} style={{ width: "100%", maxWidth: 480 }} onClick={e => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <h3 style={{ margin: 0, color: t.text, fontSize: 18, display: "flex", alignItems: "center", gap: 8 }}>
            <Icon name="shield" size={20} color={t.accent} /> Admin Transfer
          </h3>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer" }}>
            <Icon name="close" size={18} color={t.textSec} />
          </button>
        </div>

        <div style={{ padding: "12px 16px", borderRadius: 10, background: t.surface2, marginBottom: 20, fontSize: 13, color: t.textSec }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
            <span>Current Admin</span>
            <span style={{ fontWeight: 600, color: t.text }}>{entity?.adminEmail || "Not set"}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <span>{entityType === "group" ? "Group" : "Pool"}</span>
            <span style={{ fontWeight: 600, color: t.text }}>{entity?.title}</span>
          </div>
        </div>

        {isAdmin && (
          <div style={{ marginBottom: 20, paddingBottom: 20, borderBottom: `1px solid ${t.border}` }}>
            <h4 style={{ margin: "0 0 10px", fontSize: 14, fontWeight: 600, color: t.text }}>Generate Access Key</h4>
            <p style={{ fontSize: 12, color: t.textMut, marginBottom: 12 }}>Generate a one-time key to transfer admin rights. Key expires in 24 hours.</p>
            {generatedKey ? (
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <div style={{
                  flex: 1, padding: "10px 14px", background: t.accentBg, border: `1px solid ${t.accent}33`,
                  borderRadius: 8, fontFamily: "monospace", fontSize: 15, fontWeight: 700, color: t.accent,
                  letterSpacing: 2, textAlign: "center",
                }}>
                  {generatedKey}
                </div>
                <Btn t={t} size="sm" variant="secondary" onClick={handleCopyKey}>
                  <Icon name={copied ? "check" : "copy"} size={14} color={copied ? G.green : t.textSec} />
                  {copied ? "Copied!" : "Copy"}
                </Btn>
              </div>
            ) : (
              <Btn t={t} onClick={handleGenerateKey} disabled={loading}>
        ...
        <div>
          <h4 style={{ margin: "0 0 10px", fontSize: 14, fontWeight: 600, color: t.text }}>Use Access Key</h4>
          <p style={{ fontSize: 12, color: t.textMut, marginBottom: 12 }}>Enter an access key to become the new admin.</p>
          <div style={{ display: "flex", gap: 8 }}>
            <Input label="" value={inputKey} onChange={setInputKey} placeholder="Enter access key…" t={t} style={{ flex: 1, marginBottom: 0 }} />
            <Btn t={t} onClick={handleUseKey} disabled={loading || !inputKey.trim()} style={{ alignSelf: "flex-start" }}>
...
// ─── SHARE POOL MODAL ────────────────────────────────────────────────────────
function SharePoolModal({ t, pool, user, addToast, onClose }) {
  const [copied, setCopied] = useState(false);
  const [regenerating, setRegenerating] = useState(false);
  const poolAdmin = isPoolAdmin(pool, user?.uid);
  const shareUrl = pool?.shareId ? getShareUrl(pool.shareId) : "";
  const sharingEnabled = pool?.sharingEnabled !== false;
  ...
  function handleWhatsApp() {
    const text = `Join the pool "${pool.title}" — ${shareUrl}`;
    window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, "_blank");
  }

  function handleEmail() {
    const subject = `Join Pool: ${pool.title}`;
    const body = `Hi,\n\nYou've been invited to join the pool "${pool.title}".\n\nClick this link to access: ${shareUrl}\n\nBest regards`;
  ...
  async function handleWebShare() {
    if (navigator.share) {
      try {
        await navigator.share({ title: pool.title, text: `Join the pool "${pool.title}"`, url: shareUrl });
  ...
  async function toggleSharing() {
    if (!poolAdmin) { addToast("Only the pool admin can change sharing settings", "error"); return; }
    try {
      await updateDoc(doc(db, "pools", pool.id), { sharingEnabled: !sharingEnabled });
      addToast(sharingEnabled ? "Sharing disabled" : "Sharing enabled");
    } catch (e) { addToast(e.message, "error"); }
  }

  async function regenerateLink() {
    if (!poolAdmin) { addToast("Only the pool admin can regenerate links", "error"); return; }
    setRegenerating(true);
    try {
      const newShareId = genShareId();
      await updateDoc(doc(db, "pools", pool.id), { shareId: newShareId });
      addToast("Share link regenerated");
    } catch (e) { addToast(e.message, "error"); }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 500, padding: 20 }} onClick={onClose}>
      <Card t={t} style={{ width: "100%", maxWidth: 460 }} onClick={e => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <h3 style={{ margin: 0, color: t.text, fontSize: 18, display: "flex", alignItems: "center", gap: 8 }}>
            <Icon name="share" size={20} color={t.accent} /> Share Pool
          </h3>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer" }}>
            <Icon name="close" size={18} color={t.textSec} />
          </button>
        ...
        {!sharingEnabled ? (
          <div style={{ textAlign: "center", padding: "24px 0" }}>
            ...
            <p style={{ color: t.textSec, fontSize: 14, marginBottom: 16 }}>Sharing is currently disabled for this pool.</p>
            {poolAdmin && <Btn t={t} onClick={toggleSharing}>Enable Sharing</Btn>}
          </div>
        ) : (
          <>
            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 12, fontWeight: 500, color: t.textSec, display: "block", marginBottom: 6 }}>Share Link</label>
              <div style={{
                display: "flex", alignItems: "center", gap: 8, padding: "10px 14px",
                background: t.surface2, borderRadius: 10, border: `1px solid ${t.border}`,
              }}>
                <Icon name="link" size={15} color={t.textMut} />
                <span style={{ flex: 1, fontSize: 13, color: t.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{shareUrl}</span>
                <button onClick={handleCopy} style={{
                  background: copied ? `${G.green}22` : t.surface, border: `1px solid ${copied ? G.green : t.border}`,
                  borderRadius: 6, padding: "4px 10px", cursor: "pointer", display: "flex", alignItems: "center", gap: 4,
                  fontSize: 12, fontWeight: 600, color: copied ? G.green : t.textSec, transition: "all .2s",
                }}>
                  <Icon name={copied ? "check" : "copy"} size={12} color={copied ? G.green : t.textSec} />
                  {copied ? "Copied!" : "Copy"}
                </button>
              </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
              ...
              <button onClick={handleEmail} style={{
                display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
                padding: "12px 16px", borderRadius: 10, border: "none", cursor: "pointer",
                background: G.blue, color: "#fff", fontWeight: 600, fontSize: 13,
                transition: "all .15s",
              }}
                onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-1px)"; e.currentTarget.style.filter = "brightness(1.1)"; }}
                onMouseLeave={e => { e.currentTarget.style.transform = ""; e.currentTarget.style.filter = ""; }}
              >
                <Icon name="mail" size={15} color="#fff" /> Email
              </button>
              <button onClick={handleWebShare} style={{
                gridColumn: "1 / -1",
                display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
                padding: "12px 16px", borderRadius: 10, border: `1px solid ${t.border}`, cursor: "pointer",
                background: t.surface2, color: t.text, fontWeight: 600, fontSize: 13,
                transition: "all .15s",
              }}
                onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-1px)"; e.currentTarget.style.background = t.surface; }}
                onMouseLeave={e => { e.currentTarget.style.transform = ""; e.currentTarget.style.background = t.surface2; }}
              >
                <Icon name="share" size={15} color={t.textSec} /> Share via Other Apps
              </button>
            </div>

            {poolAdmin && (
              <div style={{ borderTop: `1px solid ${t.border}`, paddingTop: 16, display: "flex", gap: 10, justifyContent: "space-between", alignItems: "center", flexWrap: "wrap" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 13, color: t.textSec }}>Sharing</span>
                  <div onClick={toggleSharing} style={{
                    width: 44, height: 24, borderRadius: 99, background: sharingEnabled ? t.accent : t.surface2,
                    position: "relative", cursor: "pointer", transition: "background .2s", flexShrink: 0,
                  }}>
                    <div style={{
                      position: "absolute", top: 2, left: sharingEnabled ? 22 : 2,
                      width: 20, height: 20, borderRadius: "50%", background: "#fff",
                      transition: "left .2s", boxShadow: "0 1px 4px rgba(0,0,0,.2)",
                    }} />
                  </div>
                </div>
                <Btn t={t} size="sm" variant="secondary" onClick={regenerateLink} disabled={regenerating}>
                  <Icon name="refresh" size={13} color={t.textSec} /> Regenerate Link
                </Btn>
              </div>
            )}
          </>
...
// ─── HOME PAGE ───────────────────────────────────────────────────────────────
function HomePage({ t, groups, setPage, setSelectedGroup, addToast, addNotif, user, pools }) {
  ...
  function openEdit(g, e) {
    e.stopPropagation();
    if (!isGroupAdmin(g, user.uid)) { addToast("Only the group admin can edit this group", "error"); return; }
    setEditGroup(g); setForm({ title: g.title, description: g.description }); setMembers([...g.memberList]); setImportResult(null); setFormErrors({}); setShowModal(true);
  }
  ...
  async function saveGroup() {
    ...
    try {
      if (editGroup) {
        if (!isGroupAdmin(editGroup, user.uid)) { addToast("Only the group admin can edit this group", "error"); return; }
        
        const batch = writeBatch(db);
        batch.update(doc(db, "groups", editGroup.id), { title: form.title.trim(), description: form.description.trim(), memberList: members });

        // Clean up responses for removed members
        const newEmails = new Set(members.map(m => m.email?.toLowerCase()));
        const removedEmails = (editGroup.memberList || []).filter(m => m.email && !newEmails.has(m.email.toLowerCase())).map(m => m.email.toLowerCase());
        
        if (removedEmails.length > 0) {
          const groupPools = pools[editGroup.id] || [];
          for (const pool of groupPools) {
            const respQuery = query(collection(db, "responses"), where("poolId", "==", pool.id));
            const respSnap = await getDocs(respQuery);
            const poolRef = doc(db, "pools", pool.id);
            let poolUpdated = false;
            let currentOptions = JSON.parse(JSON.stringify(pool.options));

            respSnap.docs.forEach(rDoc => {
              const rData = rDoc.data();
              if (removedEmails.includes(rData.email?.toLowerCase())) {
                batch.delete(rDoc.ref);
                const optIndex = currentOptions.findIndex(o => o.name === rData.allotted);
                if (optIndex !== -1 && currentOptions[optIndex].filledSeats > 0) {
                  currentOptions[optIndex].filledSeats -= 1;
                  poolUpdated = true;
                }
              }
            });
            if (poolUpdated) batch.update(poolRef, { options: currentOptions });
          }
        }
        await batch.commit();
  ...
  function deleteGroup(g, e) {
    e.stopPropagation();
    if (!isGroupAdmin(g, user.uid)) { addToast("Only the group admin can delete this group", "error"); return; }
    setConfirmDel(g);
  }
  ...
  async function confirmDeleteGroup() {
    try {
      ...
      // Find all pools belonging to this group
      const poolsSnap = await getDocs(query(collection(db, "pools"), where("groupId", "==", confirmDel.id)));
      const poolIds = [];
      poolsSnap.docs.forEach(pDoc => {
        poolIds.push(pDoc.id);
        batch.delete(pDoc.ref);
      });

      // Find all responses belonging to those pools
      for (const poolId of poolIds) {
        const respSnap = await getDocs(query(collection(db, "responses"), where("poolId", "==", poolId)));
        respSnap.docs.forEach(rDoc => batch.delete(rDoc.ref));
      }

      // Delete the group itself
      batch.delete(doc(db, "groups", confirmDel.id));

      await batch.commit();
      addToast("Group and all related data deleted");
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 18 }}>
        {filtered.map(g => {
          const admin = isGroupAdmin(g, user.uid);
          return (
            <Card key={g.id} t={t} onClick={() => { setSelectedGroup(g); setPage("group"); }}>
                  ...
                  {admin && (
                    <Badge color="#92400e" bg="#fef3c7">
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}>
                        <Icon name="shield" size={9} color="#92400e" /> Admin
                      </span>
                    </Badge>
                  )}
                  {admin ? (
                    <>
                      ...
                      <button onClick={e => { e.stopPropagation(); setTransferEntity(g); }} style={{ background: "none", border: "none", cursor: "pointer", padding: 4 }} title="Transfer Admin"><Icon name="key" size={15} color={G.amber} /></button>
                    </>
                  ) : (
                    <button onClick={e => { e.stopPropagation(); setTransferEntity(g); }} style={{ background: "none", border: "none", cursor: "pointer", padding: 4 }} title="Use Access Key"><Icon name="key" size={15} color={t.textMut} /></button>
                  )}
                </div>
              </div>
              ...
              <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
                ...
                {g.adminEmail && <span style={{ fontSize: 11, color: t.textMut }}>by {g.adminEmail.split("@")[0]}</span>}
              </div>
            </Card>
          );
        })}
      ...
      {confirmDel && <ConfirmModal t={t} title="Delete Group" message={`Are you sure you want to delete "${confirmDel.title}"? This will also delete all pools and responses in this group. This action cannot be undone.`}
        onCancel={() => setConfirmDel(null)} onConfirm={confirmDeleteGroup} />}

      {transferEntity && (
        <AdminTransferModal
          t={t} entity={transferEntity} entityType="group" user={user}
          addToast={addToast} addNotif={addNotif} onClose={() => setTransferEntity(null)}
        />
      )}
    </div>
  );
...
// ─── GROUP PAGE ──────────────────────────────────────────────────────────────
function GroupPage({ t, group, pools, setPage, setSelectedPool, addToast, addNotif, user, joinRequests }) {
  const groupPools = pools[group.id] || [];
  const [confirmDel, setConfirmDel] = useState(null);
  const [sharePool, setSharePool] = useState(null);
  const [transferEntity, setTransferEntity] = useState(null);

  const groupAdmin = isGroupAdmin(group, user?.uid);

  function deletePool(pool) {
    if (!isPoolAdmin(pool, user?.uid) && !groupAdmin) {
      addToast("Only the pool or group admin can delete this pool", "error"); return;
    }
    setConfirmDel(pool);
  }
  ...
  async function confirmDeletePool() {
    try {
      ...
      // Delete all responses for this pool
      const respSnap = await getDocs(query(collection(db, "responses"), where("poolId", "==", confirmDel.id)));
      respSnap.docs.forEach(rDoc => batch.delete(rDoc.ref));

      // Delete the pool
      batch.delete(doc(db, "pools", confirmDel.id));

      await batch.commit();
      addToast("Pool and related responses deleted");
  ...
  return (
    <div style={{ maxWidth: 1000, margin: "0 auto", padding: "32px 24px" }}>
      ...
      <JoinRequestsPanel t={t} group={group} user={user} joinRequests={joinRequests} addToast={addToast} addNotif={addNotif} />
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          {group.adminEmail && <p style={{ margin: "2px 0 0", color: t.textMut, fontSize: 12 }}>Admin: {group.adminEmail}</p>}
        </div>
        {groupAdmin && (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 18 }}>
        {groupPools.map(pool => {
          ...
          const poolAdm = isPoolAdmin(pool, user?.uid);
          const canManage = poolAdm || groupAdmin;
          return (
            <Card key={pool.id} t={t}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <StatusBadge pool={pool} t={t} />
                  {poolAdm && <Badge color="#065f46" bg="#d1fae5"><span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}><Icon name="shield" size={8} color="#065f46" /> Admin</span></Badge>}
                </div>
                <div style={{ display: "flex", gap: 4 }}>
                  <button onClick={() => setSharePool(pool)} style={{ background: "none", border: "none", cursor: "pointer", padding: 3 }} title="Share Pool"><Icon name="share" size={15} color={G.blue} /></button>
                  {canManage && (
                    <>
                      <button onClick={() => { setSelectedPool(pool); setPage("createPool"); }} style={{ background: "none", border: "none", cursor: "pointer", padding: 3 }}><Icon name="edit" size={15} color={t.textSec} /></button>
                      <button onClick={() => deletePool(pool)} style={{ background: "none", border: "none", cursor: "pointer", padding: 3 }}><Icon name="trash" size={15} color={G.red} /></button>
                    </>
                  )}
                  <button onClick={() => setTransferEntity(pool)} style={{ background: "none", border: "none", cursor: "pointer", padding: 3 }} title={canManage ? "Transfer Admin" : "Use Access Key"}><Icon name="key" size={14} color={canManage ? G.amber : t.textMut} /></button>
                </div>
              </div>
        ...
        {groupPools.length === 0 && (
          <div style={{ gridColumn: "1/-1", textAlign: "center", padding: "60px 0", color: t.textSec }}>
            ...
            <p style={{ fontSize: 13, marginTop: 6 }}>{groupAdmin ? "Create your first allocation pool" : "No pools have been created in this group yet"}</p>
          </div>
        )}
      </div>

      {confirmDel && <ConfirmModal t={t} title="Delete Pool" message={`Delete "${confirmDel.title}"? All options and responses will be permanently removed.`}
        onCancel={() => setConfirmDel(null)} onConfirm={confirmDeletePool} />}

      {sharePool && (
        <SharePoolModal t={t} pool={sharePool} user={user} addToast={addToast} onClose={() => setSharePool(null)} />
      )}

      {transferEntity && (
        <AdminTransferModal
          t={t} entity={transferEntity} entityType="pool" user={user}
          addToast={addToast} addNotif={addNotif} onClose={() => setTransferEntity(null)}
        />
      )}
    </div>
...
function CreatePoolPage({ t, pool, setPage, selectedGroup, addToast, addNotif, user }) {
  ...
  async function savePool(publish = false) {
    ...
    if (pool && !isPoolAdmin(pool, user?.uid) && !isGroupAdmin(selectedGroup, user?.uid)) {
      addToast("Only the pool or group admin can edit this pool", "error"); return;
    }
...
// ─── MEMBERSHIP GATE PAGE ────────────────────────────────────────────────────
function MembershipGatePage({ t, pool, group, user, addToast, addNotif, setPage, joinRequests }) {
  const [requesting, setRequesting] = useState(false);

  const existingRequest = joinRequests.find(
    jr => jr.groupId === group?.id && jr.userId === user?.uid && jr.status === "pending"
  );
  const rejectedRequest = joinRequests.find(
    jr => jr.groupId === group?.id && jr.userId === user?.uid && jr.status === "rejected"
  );

  async function handleRequestJoin() {
    if (existingRequest) { addToast("You already have a pending request", "warning"); return; }
    setRequesting(true);
    try {
      ...
      addToast("Join request sent to the group admin");
      addNotif(`Join request sent for group "${group.title}"`);
    } catch (e) { addToast(e.message, "error"); }
    setRequesting(false);
  }
  ...
  return (
    <div style={{ maxWidth: 520, margin: "80px auto", padding: "0 24px", textAlign: "center" }}>
      <div style={{
        ...
        background: `linear-gradient(135deg, ${G.amber}33, ${G.red}22)`,
        display: "flex", alignItems: "center", justifyContent: "center",
        margin: "0 auto 24px", border: `2px solid ${G.amber}44`,
      }}>
        <Icon name="lock" size={32} color={G.amber} />
      </div>
      <h2 style={{ color: t.text, fontSize: 22, fontWeight: 700, marginBottom: 8 }}>Membership Required</h2>
      <p style={{ color: t.textSec, marginBottom: 28, lineHeight: 1.6 }}>
        You must be a member of this Group to participate in this Pool.
      </p>
      ...
      <Card t={t} style={{ textAlign: "left", marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
          <div style={{
            ...
            background: `linear-gradient(135deg,${G.green}22,${G.greenL}33)`,
            display: "flex", alignItems: "center", justifyContent: "center",
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 16, color: t.text }}>{group?.title}</div>
            <div style={{ fontSize: 13, color: t.textSec }}>{group?.description}</div>
          </div>
        </div>
        <div style={{ display: "grid", gap: 8, fontSize: 13 }}>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: `1px solid ${t.border}` }}>
            <span style={{ color: t.textSec }}>Members</span>
            <span style={{ color: t.text, fontWeight: 600 }}>{group?.memberList?.length || 0}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: `1px solid ${t.border}` }}>
            <span style={{ color: t.textSec }}>Admin</span>
            <span style={{ color: t.text, fontWeight: 600 }}>{group?.adminEmail || "—"}</span>
          </div>
          {pool && (
            <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: `1px solid ${t.border}` }}>
              <span style={{ color: t.textSec }}>Pool</span>
              <span style={{ color: t.text, fontWeight: 600 }}>{pool.title}</span>
            </div>
          )}
        </div>
      </Card>
      ...
      {existingRequest ? (
        <Card t={t} style={{ background: `${G.amber}11`, border: `1px solid ${G.amber}33`, marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, color: G.amber }}>
            <Icon name="clock" size={18} color={G.amber} />
            <div style={{ textAlign: "left" }}>
              <div style={{ fontWeight: 600, fontSize: 14 }}>Request Pending</div>
              <div style={{ fontSize: 12, color: t.textSec, marginTop: 2 }}>
                Your join request is awaiting admin approval. Sent {fmtDate(existingRequest.requestedAt)}
              </div>
            </div>
          </div>
        </Card>
      ) : rejectedRequest ? (
        <Card t={t} style={{ background: `${G.red}11`, border: `1px solid ${G.red}33`, marginBottom: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, color: G.red }}>
            <Icon name="userX" size={18} color={G.red} />
            <div style={{ textAlign: "left" }}>
              <div style={{ fontWeight: 600, fontSize: 14 }}>Request Rejected</div>
              <div style={{ fontSize: 12, color: t.textSec, marginTop: 2 }}>
                Your previous join request was rejected. You may request again.
              </div>
            </div>
          </div>
        </Card>
      ...
      <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
        <Btn t={t} variant="secondary" onClick={() => setPage("home")}>
          <Icon name="back" size={14} color={t.textSec} /> Back to Groups
        </Btn>
        {!existingRequest && (
          <Btn t={t} onClick={handleRequestJoin} disabled={requesting}>
            <Icon name="userPlus" size={15} color="#fff" />
            {requesting ? "Sending..." : "Request to Join Group"}
          </Btn>
        )}
      </div>
    </div>
...
// ─── JOIN REQUESTS PANEL ─────────────────────────────────────────────────────
function JoinRequestsPanel({ t, group, user, joinRequests, addToast, addNotif }) {
  const groupAdmin = isGroupAdmin(group, user?.uid);
  if (!groupAdmin) return null;

  const pendingRequests = joinRequests.filter(jr => jr.groupId === group.id && jr.status === "pending");
  const [expanded, setExpanded] = useState(false);

  async function handleApprove(req) {
    try {
      // Add user to group memberList
      ...
      if (memberList.some(m => m.email?.toLowerCase() === req.userEmail?.toLowerCase())) {
        await updateDoc(doc(db, "joinRequests", req.id), { status: "approved", resolvedAt: new Date().toISOString(), resolvedBy: user.uid });
        addToast(`${req.userName} is already a member`);
        return;
      }
      ...
      const batch = writeBatch(db);
      batch.update(groupRef, { memberList });
      ...
      addToast(`${req.userName} has been approved and added to the group`);
      addNotif(`${req.userName} joined group "${group.title}"`);
    } catch (e) { addToast(e.message, "error"); }
  }
  ...
  async function handleReject(req) {
    try {
      ...
      addToast(`Join request from ${req.userName} rejected`);
    } catch (e) { addToast(e.message, "error"); }
  }
  ...
  return (
    <Card t={t} style={{ marginBottom: 20, border: `1px solid ${G.amber}44`, background: `${G.amber}08` }}>
      <div
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{
            width: 32, height: 32, borderRadius: 8, background: `${G.amber}22`,
            display: "flex", alignItems: "center", justifyContent: "center",
          }}>
            <Icon name="inbox" size={16} color={G.amber} />
          </div>
          <div>
            <span style={{ fontWeight: 700, fontSize: 14, color: t.text }}>
              Join Requests
            </span>
            <Badge color="#92400e" bg="#fef3c7" style={{ marginLeft: 8 }}>{pendingRequests.length}</Badge>
          </div>
        </div>
        <Icon name={expanded ? "close" : "back"} size={16} color={t.textSec}
          style={{ transform: expanded ? "none" : "rotate(270deg)", transition: "transform .2s" }} />
      </div>
        ...
        <div style={{ marginTop: 16 }}>
          {pendingRequests.map(req => (
            <div key={req.id} style={{
              ...
              padding: "12px 0", borderBottom: `1px solid ${t.border}`, gap: 12, flexWrap: "wrap",
            }}>
              <div style={{ flex: 1, minWidth: 150 }}>
                <div style={{ fontWeight: 600, fontSize: 14, color: t.text }}>{req.userName}</div>
                <div style={{ fontSize: 12, color: t.textSec }}>{req.userEmail}</div>
                <div style={{ fontSize: 11, color: t.textMut }}>{fmtDate(req.requestedAt)}</div>
              </div>
              <div style={{ display: "flex", gap: 6 }}>
                <Btn t={t} size="sm" onClick={() => handleApprove(req)}>
                  <Icon name="userCheck" size={13} color="#fff" /> Approve
                </Btn>
                <Btn t={t} size="sm" variant="danger" onClick={() => handleReject(req)}>
                  <Icon name="userX" size={13} color="#fff" /> Reject
                </Btn>
              </div>
            </div>
          ))}
...
// ─── LIVE POOL PAGE ──────────────────────────────────────────────────────────
function LivePoolPage({ t, pool, setPage, pools, selectedGroup, responses, user, addToast, addNotif, groups, joinRequests }) {
  ...
  // Resolve the group for this pool (it may come from share link without selectedGroup)
  const resolvedGroup = selectedGroup
    ? (groups?.find(g => g.id === selectedGroup.id) || selectedGroup)
    : groups?.find(g => g.id === currentPool?.groupId);

  // Membership check
  const isMember = isGroupMember(resolvedGroup, user);

  // If user is not a member, show membership gate
  if (!isMember) {
    return (
      <MembershipGatePage
        t={t} pool={currentPool} group={resolvedGroup} user={user}
        addToast={addToast} addNotif={addNotif} setPage={setPage}
  ...
  async function submit() {
    ...
    // Double-check membership before submitting
    if (!isGroupMember(resolvedGroup, user)) {
      addToast("You must be a member of this group to participate", "error");
      return;
    }
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        // Verify membership inside transaction by reading group
        const groupRef = doc(db, "groups", resolvedGroup.id);
        const gDoc = await transaction.get(groupRef);
        if (!gDoc.exists()) throw new Error("Group not found");
        const gData = gDoc.data();
        const memberEmails = (gData.memberList || []).map(m => m.email?.toLowerCase());
        const isAdm = gData.adminId === user.uid;
        if (!isAdm && !memberEmails.includes(user.email?.toLowerCase())) {
          throw new Error("You are not a member of this group. Access denied.");
        }
...
function StatsPage({ t, groups, pools, responses }) {
  ...
  const stats = [
    { label: "Total Groups", value: groups.length, icon: "users", color: G.blue },
    { label: "Total Pools", value: allPools.length, icon: "pool", color: G.green },
    { label: "Active Pools", value: inProgress, icon: "clock", color: G.amber },
    { label: "Completed", value: completed, icon: "check", color: G.purple },
    { label: "Total Members", value: totalMembers, icon: "user", color: G.greenL },
    { label: "Responses", value: totalResponses, icon: "chart", color: "#e11d48" },
    { label: "Available Seats", value: availableSeats, icon: "lock", color: "#8b5cf6" },
    { label: "Filled Seats", value: filledSeats, icon: "check", color: G.green },
  ];
...
export default function App() {
  ...
  const [sharePoolResolved, setSharePoolResolved] = useState(false);
  ...
  useEffect(() => {
    ...
    const unsubJoinReq = onSnapshot(collection(db, "joinRequests"), snap => {
      setJoinRequests(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });

    return () => { unsubGroups(); unsubPools(); unsubResp(); unsubJoinReq(); };
  }, [user]);

  // Handle share link URL: ?pool=SHARE_ID
  useEffect(() => {
    if (!user || sharePoolResolved) return;
    const params = new URLSearchParams(window.location.search);
    const shareId = params.get("pool");
    if (!shareId) { setSharePoolResolved(true); return; }

    // Wait for pools to load, then find the matching pool
    const allPoolsList = Object.values(pools).flat();
    if (allPoolsList.length === 0) return; // Pools haven't loaded yet

    const matchedPool = allPoolsList.find(p => p.shareId === shareId);
    if (matchedPool) {
      if (matchedPool.sharingEnabled === false) {
        addToast("This pool's sharing has been disabled", "error");
      } else {
        const matchedGroup = groups.find(g => g.id === matchedPool.groupId);
        if (matchedGroup) {
          setSelectedGroup(matchedGroup);
          setSelectedPool(matchedPool);
          setPage("livePool");
          addToast(`Opened shared pool: ${matchedPool.title}`);
        }
      }
      // Clean the URL
      window.history.replaceState({}, "", window.location.pathname);
    } else {
      addToast("Shared pool not found or link is invalid", "error");
      window.history.replaceState({}, "", window.location.pathname);
    }
    setSharePoolResolved(true);
  }, [user, pools, groups, sharePoolResolved]);

  // Stale reference cleanup: if selectedGroup no longer exists, go home
  useEffect(() => {
    if (selectedGroup && groups.length > 0 && !groups.find(g => g.id === selectedGroup.id)) {
      setSelectedGroup(null);
      setSelectedPool(null);
      if (page !== "home" && page !== "stats" && page !== "settings" && page !== "profile") {
        setPage("home");
        addToast("The selected group was deleted", "warning");
      }
    }
  }, [groups]);

  // Stale reference cleanup: if selectedPool no longer exists, go to group
  useEffect(() => {
    if (selectedPool) {
      const allPoolsList = Object.values(pools).flat();
      if (allPoolsList.length > 0 && !allPoolsList.find(p => p.id === selectedPool.id)) {
        setSelectedPool(null);
        if (page === "livePool" || page === "responses" || page === "createPool") {
          setPage(selectedGroup ? "group" : "home");
          addToast("The selected pool was deleted", "warning");
        }
      }
    }
  }, [pools]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: t.bg, color: t.text, fontFamily: "'Inter','Google Sans',Roboto,Arial,sans-serif", transition: "background .3s,color .3s" }}>
      ...
      <main>
        {page === "home" && <HomePage t={t} groups={groups} setPage={setPage} setSelectedGroup={setSelectedGroup} addToast={addToast} addNotif={addNotif} user={user} pools={pools} />}
        {page === "group" && currentGroup && <GroupPage t={t} group={currentGroup} pools={pools} setPage={setPage} setSelectedPool={setSelectedPool} addToast={addToast} addNotif={addNotif} user={user} joinRequests={joinRequests} />}
        {page === "createPool" && <CreatePoolPage t={t} pool={selectedPool} setPage={setPage} selectedGroup={currentGroup} addToast={addToast} addNotif={addNotif} user={user} />}
        {page === "livePool" && selectedPool && <LivePoolPage t={t} pool={selectedPool} setPage={setPage} pools={pools} selectedGroup={currentGroup} responses={responses} user={user} addToast={addToast} addNotif={addNotif} groups={groups} joinRequests={joinRequests} />}
        {page === "responses" && <ResponsesPage t={t} pool={selectedPool} setPage={setPage} responses={responses} groups={groups} pools={pools} />}
